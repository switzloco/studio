...
const scoreDailyVFTool = ai.defineTool(
  {
    name: 'score_daily_vf',
    description: `Calculates today's Visceral Fat score using the Hourly Metabolic Partitioning Engine (v3.0 Balboa–Hopper). Call this at end-of-day or when the user asks for their daily score. Score is normalized per-user to their fat-oxidation ceiling: 100 pts = burning 70% of the user's Alpert number in fat that day, so the scale means the same for everyone. Score = Σ_slot[(fatBurned/D)×100 − (min(fatStored, faucet)/D)×100 − (muscleLost/10)×2 + (glycogenDrawn/D)×100 × 0.30] where D = 0.70 × Alpert. UNCAPPED both directions — glycogen debt from training earns 30% credit; fat storage penalty is capped per slot at the fat oxidation rate. Behavioral penalties: 3h alcohol pause (scaled for late drinking); proportional consecutive-day alcohol penalty; seed-oil meals = −5 each. Returns the score and a detailed breakdown.`,
    inputSchema: z.object({
      userId: z.string(),
