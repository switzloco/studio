...
import { syncWithingsData, WITHINGS_SYNC_INTERVAL_MS } from '@/lib/withings-sync';
import { adminHealthService } from '@/lib/health-service-admin';

/**
 * @fileOverview Cron endpoint that syncs Withings data for all connected users.
 * GET /api/cron/withings-sync?secret=<CRON_SECRET>
 */
...
export async function GET(request: NextRequest) {
  ...
  // Query users with Withings connected
  const usersSnapshot = await firestore
    .collection('users')
    .where('connectedDevice', '==', 'withings')
    .get();
  ...
  for (const userDoc of usersSnapshot.docs) {
    ...
    if (creds.lastSyncedAt && now - creds.lastSyncedAt < WITHINGS_SYNC_INTERVAL_MS) {
  ...
  for (let i = 0; i < staleUsers.length; i += BATCH_SIZE) {
    ...
    const batchResults = await Promise.allSettled(
      batch.map(async (userId) => {
        const { success } = await syncWithingsData(userId);
    ...
    for (const result of batchResults) {
      if (result.status === 'fulfilled') {
        ...
        console.error(`[WithingsCronSync] Unexpected error for user ${batch[idx]}:`, result.reason);
      }
    }
  }
